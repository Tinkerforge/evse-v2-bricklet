...
BootloaderHandleMessageResponse get_hardware_configuration(const GetHardwareConfiguration *data, GetHardwareConfiguration_Response *response) {
	...
	if(!meter.each_value_read_once) {
		...
		switch(meter.type) {
			...
			case METER_TYPE_WM3M4:         response->energy_meter_type = EVSE_V2_ENERGY_METER_TYPE_WM3M4; break;
...
BootloaderHandleMessageResponse set_enumerate_value(const SetEnumerateValue *data) {
	if(led.enumerate_initial_time == 0) {
...
BootloaderHandleMessageResponse get_enumerate_value(const GetEnumerateValue *data, GetEnumerateValue_Response *response) {
	...
	if(led.enumerate_start_time != 0) {
		...
		if(led.enumerate_value == last_enumerate_value) {
			response->value_change_time = last_enumerate_change_time;
			last_enumerate_change_time  = last_enumerate_change_time;
		} else {
