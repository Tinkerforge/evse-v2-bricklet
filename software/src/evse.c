...
void evse_load_config(void) {
	...
	if(page[EVSE_CONFIG_MAGIC6_POS] != EVSE_CONFIG_MAGIC6) {
		external_control_slot_to_default = true;
	}

	if(page[EVSE_CONFIG_MAGIC7_POS] != EVSE_CONFIG_MAGIC7) {
		phase_control.phase_switch_wait_time = EVSE_V2_PHASE_SWITCH_WAIT_TIME_DEFAULT;
	} else {
...
void evse_set_cp_duty_cycle(const float duty_cycle) {
	...
	if(last_duty_cycle != duty_cycle) {
		...
		if((duty_cycle != 1000) && (duty_cycle != 0) && (last_duty_cycle != 1000) && (last_duty_cycle != 0)) {
...
bool evse_is_shutdown(void) {
	...
	if((evse.shutdown_input_configuration == EVSE_V2_SHUTDOWN_INPUT_SHUTDOWN_ON_CLOSE) || (evse.shutdown_input_configuration == EVSE_V2_SHUTDOWN_INPUT_4200_WATT_ON_CLOSE)) {
		if(!XMC_GPIO_GetInput(EVSE_SHUTDOWN_PIN)) {
			new_return = true;
		}
	} else if((evse.shutdown_input_configuration == EVSE_V2_SHUTDOWN_INPUT_SHUTDOWN_ON_OPEN) || (evse.shutdown_input_configuration == EVSE_V2_SHUTDOWN_INPUT_4200_WATT_ON_OPEN)) {
		if(XMC_GPIO_GetInput(EVSE_SHUTDOWN_PIN)) {
			new_return = true;
		}
	}
...
void evse_init(void) {
	...
	evse.contactor_turn_off_time = 0;
	evse.last_duty_cycle_change_time = 0;
}
